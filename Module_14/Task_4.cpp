 ...
 *  Все входные данные, матрица a и вектор b вносятся из пользовательского ввода.  Итоговый вектор c
 *  надо вывести в консоль (std::cout). Тип данных элементов — всегда float. Желательно реализовать
...
void Task_14_4() {
    ...
    float matr[4][4] = { 0 }, vect[4] = { 0 };
    float result[4] = { 0 };

    std::cout << "Введите 16 чисел матрицы: ";
    for (int i = 0; i < 4; i++)
        for (int j = 0; j < 4; j++)
            std::cin >> matr[i][j];
    
    std::cout << "Введите 4 числа вектора: ";
    for (int i = 0; i < 4; i++)
        std::cin >> vect[i];
